...
router.get("/get/:id", async (req, res) => {
    const deed_id = req.params.id
    try{
        const comments = await pool.query("SELECT users.name, users.profile_picture, comment FROM comments INNER JOIN deeds ON comments.deed_id = deeds.id INNER JOIN users ON comments.comment_by_id = users.id WHERE deed_id = $1",
            [deed_id])
